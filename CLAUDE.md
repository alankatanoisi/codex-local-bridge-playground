...
Plan of record: `docs/codex-bridge-runner-roadmap.html` (Part 5). Current state is tracked in README.md. The Phase 3
native implementation has landed: `src/runner/model-client.js` is a native Responses client over
`codex-transport.js`, history is native Responses items, and goldens plus an offline mock-SSE end-to-end loop run
without credentials. `test/runner/codex-fence.test.js` guards the active Codex path against Anthropic wire shapes
(see Part 2 decision, 2026-07-10). Stage 7 pricing/docs/archive work is complete in the working tree; its bounded
live read-only proof remains before Phase 3 is closed.
...
- This repo is about OpenAI transport with **native Responses items** as internal conversation state (not Anthropic
  blocks). Phase 3 rewrote the model client, run loop, pipeline boundary, and persistence paths accordingly; native
  compaction remains a focused follow-up.
  Resist refactors that reintroduce Anthropic wire shapes into the active Codex path.
- `gpt-5.5` cost output uses public OpenAI API Standard short-context rates as a **reference only**. It must stay
  labeled `not subscription billing`; the rates do not establish billing for a ChatGPT Business access token.
...
- `src/runner/run.js`: main runner loop; history is native Responses items.
- `src/runner/model-client.js`: native Responses model client over `codex-transport.js`.
